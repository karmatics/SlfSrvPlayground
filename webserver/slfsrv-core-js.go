...
type JsCoreTemplateParams struct {
	SecretKey         string
	OS                string
	Port              string
	RootPath          string
	...
	Self              string
	JsonJS            string
	NativePromiseOnly string
}

func writeCoreJsFile(params JsCoreTemplateParams, w http.ResponseWriter) {
	var s string = selfServingJsSrc()
	tpl, err := template.New("js").Parse(s)
	if err != nil {
		http.Error(w, "500 Internal Error : Error while generating JS core template.", 500)
		fmt.Println(err)
		return
	}
	...
	if err != nil {
		http.Error(w, "500 Internal Error : Error while generating JS core file.", 500)
		fmt.Println(err)
	}
}
...
(function () {
    ...
    SLFSRV.callServer = function(funcName,args,onSuccess,onError,timeout) {
        ...
        var secret = (SLFSRV.SECRET_KEY!="")?"/"+SLFSRV.SECRET_KEY:""
        sendRequest("/call" + secret + "/" + funcName + "/" + timeout,handleRequest,jsonArgs);
    };
    ...
    function keep_alive_server_forever() {
        ...
        setTimeout(keep_alive_server_forever,2000);
    }
